...
def Mutate(Sequence):
    ...
    for i in range(len(Aminoacids)):
        Sequences.add(Aminoacids[i] + Sequence[1] + Sequence[2])
        Sequences.add(Sequence[0] + Aminoacids[i] + Sequence[2])
        Sequences.add(Sequence[0] + Sequence[1] + Aminoacids[i])
    return Sequences
...
def extend_Sequence(lQ , rQ , lD , rD , DataBaseSeq , QuerySeq , visited , threshold):
    score = 0
    for i in range(3):
        score += blosum62[(QuerySeq[lQ + i] , DataBaseSeq[lD + i])]
    op1 = 1
    op2 = 1
    while op1 == 1 or op2 == 1 :
        if op1 == 1 :
            if lQ - 1 >= 0 and lD - 1 >= 0 :
                if visited[lD - 1] == 0:
                    if score + blosum62[(QuerySeq[lQ - 1] , DataBaseSeq[lD - 1])] >= threshold :
                        visited[lD - 1] = 1
                        score += blosum62[(QuerySeq[lQ - 1] , DataBaseSeq[lD - 1])]
                        lD-=1
                        lQ-=1
                    else:
                        op1 = 0
        ...
        if op2 == 1 :
            if rQ + 1 < len(QuerySeq) and rD + 1 < len(DataBaseSeq) :
                if visited[rD + 1] == 0:
                    if score + blosum62[(QuerySeq[rQ + 1] , DataBaseSeq[rD + 1])] >= threshold :
                        visited[rD + 1] = 1
                        score += blosum62[(QuerySeq[rQ + 1] , DataBaseSeq[rD + 1])]
                        rD+=1
                        rQ+=1
                    else:
                        op2 = 0
...
def HSP(hits , DataBaseSeq , QuerySeq , threshold):
    visited = [0 for i in range(len(DataBaseSeq))]
    HSP_LIST = []
    for i in range(len(QuerySeq)):
        if i + 2 < len(QuerySeq):
            for j in range(len(hits)):
                if QuerySeq[i:i+3] == DataBaseSeq[hits[j][0]:(hits[j][1]+1)] :
                   c = 0
                   for x in range(3):
                       if visited[hits[j][0] + x] == 0 :
                           c+=1
                   if c == 3 :
                      HSP_LIST.append(extend_Sequence(i , i+2 , hits[j][0] , hits[j][1] , DataBaseSeq , QuerySeq , visited , threshold))
    return HSP_LIST
########################################################################
